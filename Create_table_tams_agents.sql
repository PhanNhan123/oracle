Viết thu tuc quét tất cà đai lý đã nghỉ viec có status = 99, làm chức năng phục hồi đại lý(update thành status 01, đồng thồi thêm cột ngày phục hồi (date_recover) --> Time line tính từ 21-Dec --> 28-Dec
Design Form, loading form hiển thị 20 dòng đại lý,, nút search theo tên đại lý , và mã đại lý --> time line:  29-Dec --> 4-Jan-2023



-- Create table
drop table  AMS.TAMS_AGENTS;
-- show tablespace file
...
CREATE TABLESPACE IX_MEDIUM
  DATAFILE 'tbs_ix_medium_01.dat' 
    SIZE 20M
  ONLINE;
...
CREATE TABLESPACE TB_SMALL
  DATAFILE 'tbs_small_01.dat' 
    SIZE 20M
  ONLINE;
...
CREATE TABLESPACE IX_SMALL
  DATAFILE 'tbs_ix_small_01.dat' 
    SIZE 20M
  ONLINE;
...
alter table AMS.TAMS_AGENTS
  add constraint AGE_CAN_FK foreign key (CAN_NUM)
  references AMS.TAMS_CANDIDATES (CAN_NUM);
alter table AMS.TAMS_AGENTS
  add constraint AGE_RAN_FK foreign key (RANK_CD)
  references AMS.TAMS_RANKS (RANK_CD);
alter table AMS.TAMS_AGENTS
  add constraint AGE_SGR_FK foreign key (RPT_TO_GRP)
  references AMS.TAMS_STRU_GROUPS (STRU_GRP_CD);
alter table AMS.TAMS_AGENTS
  add constraint AGE_STA_FK foreign key (STAT_CD)
  references AMS.TAMS_STATUS (STAT_CD);


select * from tams_agents;